r"""
bulk_lock.py -- Kompatibilitaets-Shim (lock-master Stack)

Die Implementierung liegt seit der Stack-Zerlegung (2026-07-26) in
    pure-locking/bulk_lock.py

Diese Datei haelt die flache Einstiegsflaeche des Modulroots stabil:
`import bulk_lock` und `python bulk_lock.py` funktionieren unveraendert -- das ist
...
_REAL = Path(__file__).resolve().parent / "pure-locking" / "bulk_lock.py"
